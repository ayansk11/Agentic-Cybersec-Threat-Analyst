...
class TestParseLLMJson:
    ...
    def test_json_with_think_tags(self):
        ...
        result = _parse_llm_json('<think>reasoning here</think>{"summary": "after thinking"}')
...
class TestAttackClassifierAgent:
    ...
    def test_agent_returns_techniques(
        self, mock_get_llm, mock_rag, sample_state, sample_extracted_info
    ):
        mock_llm = MagicMock()
        response = MagicMock()
...
class TestPlaybookGeneratorAgent:
    ...
    def test_agent_generates_playbook_and_sigma(
        self, mock_get_llm, sample_state, sample_extracted_info
    ):
        mock_llm = MagicMock()
